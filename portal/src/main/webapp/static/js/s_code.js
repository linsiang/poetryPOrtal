...
if (document.location.host.toLowerCase().indexOf("elong") > 0 || document.location.host.toLowerCase().indexOf("hzypms") > 0) {
    if (document.location.host.toLowerCase().indexOf("net") != -1) {
        s_account = "elongnet"
    }
    ;
// if (document.location.host.toLowerCase().indexOf("big5") != -1) { s_account="elongbig5"};
    if (document.location.host.toLowerCase().indexOf("fer") != -1) {
        s_account = "elongtest"
    }
    ;
    s = s_gi(s_account);
    ...
    function s_doPlugins(s) {
        ...
        if (s.channel == "hotels") {
            ...
            var gbhsource = intls;
            if (semid == 'bidu') {
                gbhsource = 'baidu|' + semtcid
            }
            if (semid == 'goog') {
                gbhsource = 'goog|' + semtcid
            }
            if (ibanid) {
                gbhsource = ibanid;
            }
 ...
 */
    s.getElongPageName = function (url, host) {
        ...
        if (s.getQueryParam('class')) {
            pageQual += ':return';
        }
        if (s.getQueryParam('hotelid')) {
            pageQual += ':' + s.getQueryParam('hotelid');
        }
        if (s.getQueryParam('prodid')) {
            pageQual += ':' + s.getQueryParam('prodid');
        }
        if (s.getQueryParam('fid')) {
            pageQual += ':' + s.getQueryParam('fid');
        }
        if (s.getQueryParam('tid')) {
            pageQual += ':' + s.getQueryParam('tid');
        }
        if (s.getQueryParam('uid')) {
            pageQual += ':' + s.getQueryParam('uid');
        }
        if (s.getQueryParam('Rdrectreferer')) {
            pageQual += ':entry';
        }
